...
const StarItemResult = ({
    ...
    return (
        <div className={'stars-s5-game__star _result' + (pickedStars.includes(starId) ? ' _active' : '') + (resultNumber === starId ? ' _win' : '') + (resultNumber === starId && betAmount * betMultiply > 99 && pickedStars.includes(starId) ? ' _long' : '')}>
            ...
            {pickedStars.includes(starId) && resultNumber !== starId 
            ...
            <span>{resultNumber === starId && pickedStars.includes(starId) ? betAmount * betMultiply : 0}</span>
        </div>
    );
};

export default StarItemResult;