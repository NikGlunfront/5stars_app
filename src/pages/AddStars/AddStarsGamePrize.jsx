...
const AddStarsGamePrize = ({
    ...
    const revealPrize = async () => {
        if (!isGamePrizeFinished && prizeId) {
            ...
            if (prizeVal) {
                ...
                dispatch(resetPrize())
                sendAlert(`Баланс пополнен на ${prizeVal} Stars`)
            }
        }
    }
    ...
    return (
        <BoxWrapper className={'s5-game add-star-game add-star-game_prize' + (prizeData?.error ? " _disabled" : "")}>
            ...
            >
                <div>{isGamePrizeFinished ? (pickedStarPrize ? `ADD ${activeGamePrize.filter(item => item.id === pickedStarPrize)[0]['value']} STARS` : 'ADD STARS') : 'TAP & WIN BONUS'}</div>
