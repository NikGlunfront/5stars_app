...
const AddStarsGame = ({
    ...
    return (
        <BoxWrapper className={'s5-game add-star-game' + (bonusData?.error ? " _disabled" : "")}>
            ...
            >
                <div>{isBonusGameFinished ? (pickedAddStarValue ? `ADD ${pickedAddStarValue + (pickedAddStarValue * activeBonusGame.filter(item => item.id === pickedStar)[0]['value'] / 100)} STARS` : 'ADD STARS') : 'TAP & WIN BONUS'}</div>
