...
import starWin from '../../assets/img/icons/game/bets/star_item_purple.svg'
import starExpired from '../../assets/img/icons/game/bets/star_item_red.svg'

const AddStarsBetPrize = ({
    pickedStar,
    handleBet,
    isExpired = false
}) => {
    ...
    const betClick = (value) => {
        if (!isGamePrizeFinished) {
            handleBet(value)
        }
    }

    if (!activeGamePrize.length) {
        return
    }

    return (
        <div className='stars-s5-game__bet'>
            {activeGamePrize.map(star => (
                <div key={star.id} className={'stars-s5-game__star stars-s5-game__star_bonus' + (pickedStar === star.id ? ' _active' : '') + (isGamePrizeFinished && star.value > 99 ? ' _long' : '') + (isGamePrizeFinished ? ' _finished' : "")} onClick={() => betClick(star.id)}>
                    ...
                    {pickedStar === star.id && !isGamePrizeFinished
                        && <img src={starPicked} alt="" />
                    }
                    {isGamePrizeFinished && pickedStar === star.id && !isExpired
                        && <img src={starWin} alt="" />
                    }
                    {pickedStar !== star.id
                    ...
                    <span>{isGamePrizeFinished ? star.value : "?"}</span>
                </div>
            ))}
        </div>
    )
...
export default AddStarsBetPrize;
