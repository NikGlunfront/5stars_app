...
const TriesGame = ({
    ...
    const [pickedAddStarValue, setPickedAddStarValue] = useState(0)
    const [pickedTryVar, setPickedTryVar] = useState(0)
    ...
    const handlePickStarValueTries = (tries, stars) => {
        setPickedAddStarValue(stars)
        setPickedTryVar(tries)
    }
    ...
    const addTriesNoBonusHandler = async () => {
        await addTriesClick({
            ...
            value: pickedTryVar,
            params: {
                stars_cost: pickedAddStarValue
            }
        })
        sendAlert(`Your profile has been charged on ${pickedTryVar} Tries`)
    ...
    const addTriesWithBonusHandler = async () => {
        ...
        if (!isAddStarsLoading && pickedStarBet) {
            await addTriesClick({
                ...
                value: pickedTryVar,
                params: {
                    bonus_multiply: activeBonusGame.filter(item => item.id === pickedStar)[0]['value'] / 100,
                    stars_cost: pickedAddStarValue
                }
            })
            sendAlert(`Your profile has been charged on ${pickedTryVar + (activeBonusGame.filter(item => item.id === pickedStar)[0]['value'] / 100 * pickedTryVar)} Tries`)
            dispatch(resetAttBonus())
        }
    } 
    ...
    return (
        <>
            <BoxWrapper className={'s5-game add-star-game add-star-game_tries' + (isExpired ? " _disabled" : "")}>
                ...
                >
                    <div>{btnText}</div>
                    <span className={((!isBonusGameFinished && !isExpired) || !pickedAddStarValue) ? "_hidden" : ''}>
            ...
            {(isBonusGameFinished || isExpired) && 
                <div className="tries-vars">
                    {isBonusGameFinished && <p style={{textAlign: 'center', fontSize: '1.6rem', }}>Choose an amount and get the bonus <b>{activeBonusGame.filter(item => item.id === pickedStar)[0]['value']}%</b>, <br />till timer is counting down.</p>}
                    {triesVars.map(tryItem => (
                        <div 
                            key={tryItem.tries} 
                            className={"tries-item-pick" + (tryItem.tries === pickedTryVar ? " _active ": "")} 
                            onClick={() => handlePickStarValueTries(tryItem.tries, tryItem.stars)}
                        >
                            <span>{tryItem.tries} tries</span>
                            <span>{tryItem.stars}</span>
                        </div>
                    ))}
                </div>
            }
