...
const TriesGamePrize = ({
    ...
    return (
        <BoxWrapper className={'s5-game add-star-game add-star-game_tries add-star-game_tries_prize ' + (bonusData?.prize?.error ? " _disabled" : "") + (isExpired ? ' _expired' : "")}>
            ...
            {!isExpired
                ...
                >
                    <div>{isGamePrizeFinished ? (pickedStarPrize ? `ADD ${activeGamePrize.filter(item => item.id === pickedStarPrize)[0]['value']} FREE TRIES` : 'ADD TRIES') : 'TAP & WIN PRIZE'}</div>
